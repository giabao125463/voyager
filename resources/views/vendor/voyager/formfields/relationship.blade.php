    ...
    @if(class_exists($options->model))
        ...
        @elseif($options->type == 'hasOne')
            ...
            @php
                ...
                $relationshipData = (isset($data)) ? $data : $dataTypeContent;
                $relationshipColumn = $relationshipData->id;
                if (class_basename($relationshipData) === 'AnketResult'){
                    $relationshipColumn = $relationshipData->anket_access_id;
                }
                $model = app($options->model);
                $query = $model::where($options->column, '=', $relationshipColumn)->withTrashed()->first();
            @endphp

            @if(isset($query))
                @if (isset($options->options))
                    <p>{{ $options->options->{($query->{$options->label})} }}</p>
                @else
                    <p>{{ $query->{$options->label} }}</p>
                @endif
            @else
